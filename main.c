...
/*void handwhile()
{
    ...
    token *tm,*sep,*h;
    tm=start->next;
    while(strcmp(tm->tok,"while")!=0)
        tm=tm->next;
    if(strcmp(tm->tok,"while")==0)
    {   if(strcmp(tm->tok,"while")==0)///="if") ///tm IS "if" token
        {
        h=tm;
        ...
        {
            sep=sep->next;
            if(sep->tp==identifier)
            {
                IR_code(identifier,sep);
            }
            if(sep->tp==num)
            {
                if(used_num(sep))
            ...
            if(sep->tp==operate && strcmp(sep->tok,"=")!=0)
            {
            printf(" %s ",sep->tok);
            }
            if(strcmp(sep->tok,"=")==0)
